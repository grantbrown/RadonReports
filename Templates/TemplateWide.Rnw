...
out <- paste0(out, "Specifically, this figure shows the mean radon level across all the tests reported during the period for which data were available. This map shows a general, overall level of risk in an area without specifically considering the housing environment. ")
