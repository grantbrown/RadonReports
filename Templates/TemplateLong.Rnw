...
cat(paste0("\\item Among counties with at least 10 reported tests, the highest average radon concentration was observed in ", 
           ...
           commafy(round((Data_at_least_10$RawMean)[maxIdx], 1)),
           " pCi/L."))
...
\includegraphics[width=\linewidth]{SmoothedMeans.pdf}
  \caption{Smoothed mean radon level by county over all reported tests.\label{fig:means}}
  \end{minipage}  
   \hfill
  \begin{minipage}[b]{0.49\linewidth}
  \includegraphics[width=\linewidth]{SmoothedTestingRate.pdf}
...
\begin{center}
\begin{figure}[H]
  \includegraphics[width=.95\linewidth]{SmoothedNeedToTest.pdf}
