...
colnames(TableOutput) <- c("County Name", "Households", 
                           "Raw Need", "Smoothed Need", 
...
Log1pMap(Data$RawPositive / Data$RawCounts, Data$geoId, Data$LowData, 
          "Positive Testing Rate", 4, "Plots/RawPositiveRate.pdf",
          Width = 6, Height = 6)
...
Log1pMap(Data$RawNeed, 
              Data$geoId, Data$LowData, "Need to Test\n(arbitrary scale)", 1, 
...
## Need Metrics
Log1pMap(ifelse(Data$SmoothNeed > 100, 100, Data$SmoothNeed), 
              Data$geoId, Data$LowData, "Need to Test\n(arbitrary scale)", 1,
...
cat(paste0("\\textbf{Radon Undertesting Report: ", State, "}\\\\"))
...
cat(paste0("\\item Testing rates per home vary, with the lowest estimated rates in ", 
           Data$Name[minIdx], 
...
cat(paste0("\\item The county with the most tests is ", 
           ...
           commafy(max(Data$RawCounts, na.rm = TRUE))," tested homes and an estimated mean 
           radon level of ",
           round((Data$RawMean)[which.max(Data$RawCounts)], 1), 
           " pCi/L."))
...
\begin{figure}[H]
  ...
  \caption{Smoothed Need to Test metric by county. \label{fig:utest1}}
\end{figure}
\end{center}

Finally, Figure \ref{fig:utest1} shows a combined measure which attempts to capture the need to perform additional Radon tests in a given county. This Need to Test metric is designed to show high values for areas with high radon concentration, as well as low testing rates. The highest values are observed in areas with both - indicating that more tests are needed. It is also important to consider 
the radon concentrations and testing rates separately, but the Need to Test metric offers a quick visual way to highlight the areas where more testing could be the most beneficial. 

\section{Technical Notes}
\label{sec:technotessec}
Data on radon tests and mean concentrations was obtained from the CDC National Public Health Environmental Tracking Network via the Tracking API \cite{CDCEpi,mcgeehin2004national}. Census data for housin-unit adjusted comparisons were obtained from the U.S. census via the \texttt{tidycensus} package for R version 4.1.2 \cite{R-cite, R-tidycensus}. Full code and tabular versions of the data are available at {\color{red}{(insert GitHub URL)}}
...
The selected Need to Test metric is $R*log_{10}(\frac{H}{N})$ where R is the mean radon level, H is the number of housing units, and N is the number of Radon tests, adjusted to reflect the expected number of tests per 10 year period. As this metric increases, it indicates a greater need to test. The values shown in Figure \ref{fig:utest1} are scaled so that the lowest value of the Need to Test metric in the U.S. is 0 and the largest value is 100, with need values above 25\% of the national maximum capped at 100 to prevent outliers from dominating the scale. This approach can help highlight areas which may have a higher need for testing than others, but there are substantial limitations, and policy should not be based on this document in isolation. In addition to the presence of unaccounted-for-small-scale variability within states, comparisons between states may be affected by differential
data availability. In addition, the Need to Test metric presented here describes one of many possible prioritization schemes for trading off radon levels and testing rates. Alternative approaches may strike a different balance between these two measures, or prioritize high or low population areas. Direct interpretation of the units presented here is also limited, and is intended to support relative comparisons. 
...
\begin{figure}[H] 
  ...
  \caption{Raw number of radon tests per 100,000 housing units by county.}
  \end{minipage}  
  \begin{minipage}[b]{0.38\linewidth}
  \includegraphics[width=\linewidth]{RawNeedToTest.pdf}
  \caption{Raw Need to Test metric (unscaled) by county.}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.38\linewidth}
