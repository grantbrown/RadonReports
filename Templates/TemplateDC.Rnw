...
@
      ...
      {\large{ 
 <<echo = FALSE, results = "asis">>=     
cat(paste0("Radon and Radon Testing in ", State, "\\\\"))
...
Public health professionals may be interested in mean radon levels, testing rates, or a combined measure that highlights relatively fewer tests and also higher radon levels gives a summary. In all cases, caution is required in interpreting the results due to the issues highlighted in Section \ref{sec:technotessec}. In addition, the District of Colombia does not have counties over which the distribution of these quantities can be compared, so it may be of interest to compare to other state reports. 
